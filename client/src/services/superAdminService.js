...
export async function fetchCreditSettings() {
  const token = tokenStorage.getSuperAdminToken();
  if (!token) {
    throw new Error('Super admin token not available');
  }
...
export async function updateCreditSettings(payload) {
  ...
  return await apiClient.patch('/superadmin/credits/settings', payload, token);
}

export async function fetchCreditUsers() {
  const token = tokenStorage.getSuperAdminToken();
  if (!token) {
    throw new Error('Super admin token not available');
  }

  return await apiClient.get('/superadmin/credits/users', token);
}

export async function fetchCreditTransactions(userId) {
  const token = tokenStorage.getSuperAdminToken();
  if (!token) {
    throw new Error('Super admin token not available');
  }
  ...
  return await apiClient.get(`/superadmin/credits/transactions${query}`, token);
}
...
export async function allocateCredits(payload) {
  ...
  return await apiClient.post('/superadmin/credits/allocate', payload, token);
}

export async function updateUserPersonaLimit(userId, payload) {
  const token = tokenStorage.getSuperAdminToken();
  if (!token) {
    throw new Error('Super admin token not available');
  }
