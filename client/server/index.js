...
const uploadArthGangaLogo = async () => {
  try {
    ...
    if (stats.isFile()) {
      ...
      arthGangaLogoUrl = await uploadBufferToS3({
        buffer,
        mimeType: 'image/png',
        folder: 'logos',
        fileName: 'arth-ganga-eng-logo.png',
      });
      console.log(`\n✓ Arth Ganga logo uploaded to S3: ${arthGangaLogoUrl}`);
    }
  } catch (error) {
...
const defaultVideoGenerationCostEnv = Number(process.env.DEFAULT_VIDEO_GENERATION_COST || 69);
...
const normalizeAzureVideoResult = async ({
  ...
  if (normalizedStatus === 'completed' && videoId) {
    try {
      console.log(`[VIDEO JOB DEBUG] Attempting S3 upload for ${videoId}, current URL is: ${videoUrl}`);
      resolvedVideoUrl = await saveAzureVideoAsset({
        videoId,
        logoUrl,
        logoPlacement,
      });
      console.log(`[VIDEO JOB DEBUG] S3 upload successful. URL: ${resolvedVideoUrl}`);
    } catch (error) {
      console.error('[VIDEO JOB DEBUG] Azure video S3 upload failed:', error.message || error);
    }
  }
...
const uploadBufferToS3 = async ({ buffer, mimeType, folder, fileName }) => {
  const safeFileName = String(fileName || `${Date.now()}-${crypto.randomBytes(6).toString('hex')}`).replace(/[^a-zA-Z0-9._/-]/g, '-');
  const key = `${folder}/${safeFileName}`.replace(/^\/+/, '');
...
const saveLogoUpload = async (logoFile) => {
  ...
  const extension = parsed.mimeType.includes('jpeg') || parsed.mimeType.includes('jpg') ? 'jpg' : 'png';
  return uploadBufferToS3({
    buffer: parsed.buffer,
    mimeType: parsed.mimeType,
    folder: 'logos',
    fileName: `${Date.now()}-${crypto.randomBytes(6).toString('hex')}.${extension}`,
  });
};
...
const getLogoCompositeGravity = (logoPlacement) => {
  switch (String(logoPlacement || '').trim().toLowerCase()) {
    case 'top-left':
      return 'northwest';
    case 'top-right':
      return 'northeast';
...
const applyLogoOverlayToImage = async ({ imageBuffer, logoBuffer, logoPlacement }) => {
  if (!imageBuffer || !logoBuffer) {
    return imageBuffer;
  }

  const gravity = getLogoCompositeGravity(logoPlacement);
  ...
  const logoTargetWidth = Math.max(96, Math.round(longestSide * 0.18));

  const preparedLogo = await sharp(logoBuffer)
    .rotate()
    .resize({
      width: logoTargetWidth,
      height: logoTargetWidth,
      fit: 'inside',
...
const shouldApplyLogoOverlay =
  finalImageUrl &&
  logoUrl &&
  logoPlacement &&
  logoPlacement !== 'none';

  if (shouldApplyLogoOverlay) {
  try {
    console.log('[IMAGE OVERLAY] Starting local logo compositing...');
    const baseImageBuffer = await fetchImageBufferFromUrl(finalImageUrl);
    const logoBuffer = await fetchImageBufferFromUrl(logoUrl);
    const compositedImageBuffer = await applyLogoOverlayToImage({
    ...
    console.error('[IMAGE OVERLAY FAILED]', error);
  }
}
...
const createMongoStore = (db) => ({
    ...
    await appSettings.updateOne(
      { key: 'credits' },
      {
        $setOnInsert: {
          ...
          video_generation_cost: normalizeGenerationCost(defaultVideoGenerationCostEnv, 69),
          created_at: nowIso(),
        },
        $set: {
          updated_at: nowIso(),
  ...
  async getCreditSettings() {
    ...
    return {
      ...
      video_generation_cost: normalizeGenerationCost(settings?.video_generation_cost, normalizeGenerationCost(defaultVideoGenerationCostEnv, 69)),
    ...
    await db.collection('app_settings').updateOne(
      { key: 'credits' },
      {
        $set: {
          ...
          video_generation_cost: normalizeGenerationCost(videoGenerationCost, normalizeGenerationCost(current.video_generation_cost, normalizeGenerationCost(defaultVideoGenerationCostEnv, 69))),
          updated_at: nowIso(),
        },
        $setOnInsert: {
          key: 'credits',
...
app.get('/api/download-asset', authRequired, async (req, res) => {
  try {
    ...
    const isS3Host = parsedUrl.hostname === `${process.env.AWS_BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com`;
    const isLocalUpload = parsedUrl.origin === `${req.protocol}://${req.get('host')}` && parsedUrl.pathname.startsWith('/uploads/');

    if (!isCloudinaryHost && !isS3Host && !isLocalUpload) {
...
app.post('/api/generate-video', authRequired, async (req, res) => {
  const userId = req.user.id || req.user._id.toString();
  const settings = await store.getCreditSettings();
  const videoCost = normalizeGenerationCost(settings.video_generation_cost, 69);
  let chargeResult = null;
